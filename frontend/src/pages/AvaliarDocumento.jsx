  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-xl shadow-md space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {documentos.map(doc => (
          <div key={doc._id} className="border border-gray-300 rounded-lg p-4 shadow-sm hover:shadow-md transition">
            <h3 className="text-lg font-semibold text-gray-800">{doc.tipo}</h3>
            <p className="text-sm text-gray-700"><strong>Aluno:</strong> {doc.usuarioId?.nome_completo || 'Desconhecido'}</p>
            <p className="text-sm text-gray-700"><strong>Matrícula:</strong> {doc.usuarioId?.numero_matricula || 'N/A'}</p>
            <p className="text-sm text-gray-700"><strong>Status:</strong> {doc.estado}</p>
            <p className="text-sm text-gray-700"><strong>Arquivo:</strong> {doc.filename}</p>
            <button
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => atualizarStatusDocumento(documentoSelecionado._id, 'aprovado')}
                  disabled={loading}
                ...
                <button
                  onClick={() => atualizarStatusDocumento(documentoSelecionado._id, 'reprovado')}
                  disabled={loading}
